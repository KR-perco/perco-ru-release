...
$(function(){
	...
	if (url("filename") == "archive")
		archive = "Y";
	$.get("select.php", {"section": "vsya-dokumentatsiya", "archive": archive}, function(data){
		$("#section").html(data);
	});
	if (url("?section") != undefined)
	{
		window.param = url("?section");
		var jqxhr = $.get("select.php", {"section": window.param, "archive": archive}, function(data){
			$("#section").after(data);
		...
		$("#download_items").load("select_products.php", {"section": window.param, "archive": archive});
	}
	$(document).on("change", "#select_documents select", function(){
		...
		else
		{
			...
			$.get("select.php", {"section": window.param, "archive": archive}, function(data){
				$(id + "+ select").remove();
				$(id).after(data);
