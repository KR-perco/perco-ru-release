...
$APPLICATION->SetPageProperty("title", "Журнал ВУЗов");
$APPLICATION->SetPageProperty("keywords", "");
$APPLICATION->SetPageProperty("description", "");
$APPLICATION->SetTitle("Журнал ВУЗов");
...
echo '<table class="uchitelskaya_jurnal" width="100%" border="0" cellspacing="0" cellpadding="0">
		<thead>
			...
			<td id="uchitelskaya_jurnal_td_2" valign="top"><img src="/bitrix/images/icons/up_down.gif" /><br />ВУЗ</td>
	...
	for ($i=0; $i < $cnt; $i++)
	{
		...
		if ($arUser["UF_VUZ"])
		{
			$rsVUZ = CUserFieldEnum::GetList(array(), array("ID" => $arUser["UF_VUZ"]));
			$arVUZ = $rsVUZ->GetNext();
			$vuz = $arVUZ["VALUE"];
		}
		else
		...
		if ($vuz)
		{
			...
			echo '><td valign="top"><a href="kursi.php?STUDENT_ID='. $arUnicTest[$i][0]["USER_ID"].'">'.$arUser["LAST_NAME"].' '.$arUser["NAME"].' '.$arUser["SECOND_NAME"].'</a></td><td>'.$vuz.'</td>';
			...
			$arAllUserFields[$arUnicTest[$i][0]["USER_ID"]][] = $vuz;
