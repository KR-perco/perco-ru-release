...
<?
$arResult["TIMESTAMP_X"] = $arResult["ITEMS"][0]["TIMESTAMP_X"];
foreach ($arResult['CITEMS'] as $sectionName => $section) {
?>
	<h2><?=$sectionName?></h2><?
	foreach ($section as $itemName => $item) {
	?>
	<div class="element_item">
		<a href="<?if ($arItem[ID] == "21112") {echo "/resheniya/gibkiy-grafik/";} else { echo $item['URL'];}?>">
			<div class="image_icon">
				<img src="<?=$item['ICON_SRC'];?>" />
			</div>
			<div class="text_item">
				<div class="item_name <?//bottom_border?>"><?=$itemName;?></div>
			</div>
			<!--<div class="more">
				<span>подробнее</span>
				<span class="arrow"><?include($_SERVER["DOCUMENT_ROOT"]."/images/icons/arrow_mini.svg");?></span>
