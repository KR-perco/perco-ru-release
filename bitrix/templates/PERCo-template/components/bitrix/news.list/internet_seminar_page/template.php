...
<?
$imgs = [
	'Как выбрать турникет' => '/images/internet-seminary/kak-vybrat-turniket.jpg',
	'Проектирование системы безопасности на основе оборудования PERCo' => '/images/internet-seminary/proektirovanie-sistem.jpg',
	'Способы идентификации в системах PERCо' => '/images/internet-seminary/sposoby-identifikatsii.jpg',
	'Выбор программного обеспечения PERCo' => '/images/internet-seminary/vybor-po.jpg',
	'Системы безопасности PERCo' => '/images/internet-seminary/sistema-bezopasnosti.jpg',
	'Новое в архитектуре СКУД - система без выделенного сервера' => '/images/internet-seminary/novoe-v-arkhitekture.jpg'
];
?>
<div class="sem-items">
	<?
	foreach($arResult["ITEMS"] as $arItem) {
		?><pre style="display: none;"><?var_dump($arItem);?></pre><?
		if (!$arItem["PROPERTIES"]["SEMINAR"]["VALUE"])
			continue;
		$arFilter = array("IBLOCK_CODE"=>"list_seminars", "ID"=>$arItem["PROPERTIES"]["SEMINAR"]["VALUE"]);
		$arSelect = array("ID", "IBLOCK_ID", "NAME", "PREVIEW_TEXT", "DETAIL_TEXT", "PROPERTY_TIME", "PROPERTY_DURATION", "PROPERTY_TOPIC", "PROPERTY_CITY", "PROPERTY_FOR_US_P", "PROPERTY_FOR_US_U", "PROPERTY_TYPE_SEMINAR");
		...
		$arRes = $res->GetNextElement();
		$arProp = $arRes->GetProperties();
		if (empty($imgs[trim($arProp["TOPIC"]["VALUE"])])) continue;
		$datetime = $arItem["DATE_ACTIVE_TO"];
		$format = "DD.MM.YYYY HH:MI";
		$arr = ParseDateTime($datetime, $format);
	?>
		<div class="sem-item">
			<img src="<?=$imgs[trim($arProp["TOPIC"]["VALUE"])]?>">
			<div class="sem-item__text">
				<div class="sem-item__title" style="block-size: 69px;"><?=$arProp["TOPIC"]["VALUE"]?></div>
				<span class="sem-item__reg"><a href="/client/registration/vebinar.php?ID=<?=$arItem["ID"];?>" rel="nofollow" target="_blank"><?=$arr["DD"]?>.<?=$arr["MM"]?>. Регистрация на семинар</a></span>
			</div>
		</div>
	<?}?>
</div>