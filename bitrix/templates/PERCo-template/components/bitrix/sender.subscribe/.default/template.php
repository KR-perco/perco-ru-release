...
?>
<div class="bx-subscribe subscription"  id="sender-subscribe">
<?
...
?>
	<?if(isset($arResult['MESSAGE'])): CJSCore::Init(array("popup"));?>
		<div id="sender-subscribe-response-cont" style="display: none;">
			<div class="bx_subscribe_response_container">
				<div style="font-size: 16px; max-width: 320px; padding: 35px;"><?=htmlspecialcharsbx($arResult['MESSAGE']['TEXT'])?></div>
			</div>
		</div>
		<script>
			BX.ready(function(){
	...
	<form id="bx_subscribe_subform_<?=$buttonId?>" role="form" method="post" action="<?=$arResult["FORM_ACTION"]?>">
		...
		<div class="bx-input-group group">
			<input required class="bx-form-control" type="text" name="SENDER_SUBSCRIBE_EMAIL" value="<?=$arResult["EMAIL"]?>" title="<?=GetMessage("subscr_form_email_title")?>"<?= (LANGUAGE_ID == 'fr' || LANGUAGE_ID == 'it' || LANGUAGE_ID == 'es') ? ' style="height: 32px;"' :  ''; ?>>
			<label><?=htmlspecialcharsbx(GetMessage('subscr_form_email_title'))?></label>
		...
		<button class="sender-btn btn-subscribe" id="bx_subscribe_btn_<?=$buttonId?>" type="submit">
			<div>
				<img alt="<?=GetMessage("CT_BSE_SUBSCRIPTION_FORM_TITLE")?>" width="14" height="14" src="/images/icons/key.svg"><?=GetMessage("subscr_form_button")?>
			</div>
		</button>
	</form>
<?
...
?>
	...
	<form id="bx_subscribe_subform_<?=$buttonId?>" role="form" method="post" action="<?=$arResult["FORM_ACTION"]?>">
		...
		<div class="bx-input-group">
			<input class="bx-form-control" type="text" name="SENDER_SUBSCRIBE_EMAIL" value="" title="<?=GetMessage("subscr_form_email_title")?>" placeholder="<?=htmlspecialcharsbx(GetMessage('subscr_form_email_title'))?>">
		</div>
