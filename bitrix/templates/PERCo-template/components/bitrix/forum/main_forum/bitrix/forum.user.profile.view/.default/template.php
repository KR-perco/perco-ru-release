...
<?
		elseif (!empty($arResult["USER"]["WORK_COMPANY"])):
			if(is_numeric($arResult["USER"]["WORK_COMPANY"])){
				$res = CUser::GetByID($arResult["USER"]["WORK_COMPANY"]);
				$arUser = $res->Fetch();
				?>
					<a rel="nofollow" href="<?=$arUser["WORK_WWW"]?>" target="_blank"><?=$arUser["WORK_COMPANY"]?></a>
				<?
