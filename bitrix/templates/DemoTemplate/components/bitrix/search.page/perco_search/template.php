...
<?elseif($arResult["ERROR_CODE"]!=0):?>
	...
	<?/*?><br /><b><?=GetMessage("SEARCH_LOGIC")?></b></p>
...
<?elseif(count($arResult["SEARCH"])>0):?>
	<?if($arParams["DISPLAY_TOP_PAGER"] != "N") echo $arResult["NAV_STRING"]?>
		<? /* if (LANGUAGE_ID=='en') { ?><pre><? print_r($arResult["SEARCH"]); ?></pre><? } */?>
	<br /><hr />
	<?foreach($arResult["SEARCH"] as $arItem):
        ?>
        <a class="search-page-zag" href="<? echo $arItem["URL"]; ?>"><?echo $arItem["TITLE_FORMATED"]?></a>
		...
		$date_elements  = explode(".",$arItem["DATE_CHANGE"]);
		$date_per=$date_elements[2].'-'.$date_elements[1].'-'.$date_elements[0];

		$arItem["CHAIN_PATH"]=str_replace('<a href="'.GetMessage("SERCH_ARCH").'details/">'.GetMessage("PAGE_DETAIL").'</a>','<a href="'.GetMessage("SERCH_ARCH").$arItem["ITEM_ID"].'/'.$date_per.'/">'.$arItem["TITLE_FORMATED"].'</a>' ,$arItem["CHAIN_PATH"]);
