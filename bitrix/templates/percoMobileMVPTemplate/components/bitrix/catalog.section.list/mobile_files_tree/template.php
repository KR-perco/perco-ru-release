...
function getElements($iblock_id, $section_id, $archive, $with_image)
{
	...
	if (intval($rs->SelectedRowsCount()) > 0)
	{
		...
		while($ar = $rs->GetNextElement())
		{
			...
			if ($with_image == "Y")
				$list_files .= '<div class="element_item"><div><img alt="'.$name.'" src="'.$arProps["IMAGE"]["VALUE"].'"></div><a href="https://www.perco.ru'.$file.'">'.$name.'</a><div class="color">'.$fSize.' — '.$date.'</div></div>';
			else
				$list_files .= '<li><a href="https://www.perco.ru'.$file.'">'.$name.$version.'</a> <span class="color">'.$fSize.' — '.$date.'</span></li>';
		}
		if ($with_image == "Y")
