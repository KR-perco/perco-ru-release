			...
			document.querySelectorAll('.video .video-list .item').forEach(item => {
				...
				item.addEventListener('touchend', function (e) {
					...
					document.querySelector('.video .text').innerHTML = `
						...
						<div class="download">
							<a href="${items[this.dataset.id].fileLink}">Скачать</a> (${items[this.dataset.id].fileSize} MB) — ${items[this.dataset.id].fileDate}
						</div>
					`;
			...
			btn.addEventListener('navTabsPressed', async function () {
				...
				if (res.ok) {
					...
					<div class="text">
						...
						<div class="download">
							<a href="${json[0].fileLink}">Скачать</a> (${json[0].fileSize} MB) — ${json[0].fileDate}
						</div>
					</div>
