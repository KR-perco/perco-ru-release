...
<div class="video">
	...
	<div class="text">
		...
		<? if (!empty($arResult['ITEMS'][0]["DISPLAY_PROPERTIES"]['FILE']["VALUE"][0])) { ?>
			<div class="download">
				<a href="<?= $arResult['ITEMS'][0]["DISPLAY_PROPERTIES"]['FILE']["VALUE"][0] ?>">Скачать</a> (<?= round(filesize('/home/d/dc178435/public_html'.$arResult['ITEMS'][0]["DISPLAY_PROPERTIES"]['FILE']["VALUE"][0]) / 1048576, 2); ?> MB) — <?=date('d.m.Y', filemtime('/home/d/dc178435/public_html'.$arResult['ITEMS'][0]["DISPLAY_PROPERTIES"]['FILE']["VALUE"][0]));?>
			</div>
		<? } ?>
	</div>
	<div class="video-list">
		<?php
		foreach ($arResult['ITEMS'] as $i => $item) {
		if (empty($item['NAME'] || $item['DISPLAY_PROPERTIES']['DESCRIPTION']['VALUE'][0]['TEXT']) || empty($item["DISPLAY_PROPERTIES"]['YOUTUBE']["VALUE"][0]) || empty($item['DISPLAY_PROPERTIES']['IMAGE']["VALUE"][0])) {
			continue;
		}
		?>
