...
elseif (stripos($_SERVER['REQUEST_URI'], '/obuchenie/')!== false)
{
	$local = "all";
}
	...
	for ($i = 0; $i < 6; $i++)
	{
		...
		for ($j = 0; $j < 7; $j++)
		{
			...
			if(is_set($arDays[$tmpDate]))
			{
				...
				{
					$rsSeminar = CIBlockElement::GetProperty(46, $dayNews["ID"], array("sort" => "asc"), Array("CODE"=>"SEMINAR"));
					$arSeminar = $rsSeminar->Fetch();
					$rsTema = CIBlockElement::GetList(
						array(), 
						array(
						"IBLOCK_ID" => 62,
						"ID" => $arSeminar["VALUE"]
						...
						false, 
						false,
						array("PROPERTY_TOPIC")	// перечень полей необходимых в результате выборки
					);
					$arTema = $rsTema->Fetch();
					$dayNews["NAME"] = str_ireplace("#kto#", $tema, $arTema["PROPERTY_TOPIC_VALUE"]);
					$nn++;
					$eTime = "";
					$arTime = Array();
					...
					if($arParams["TITLE_LEN"]>0)
						...
						$title = $dayNews["NAME"];
					$db_iblock = CIBlockElement::GetProperty(62, $arSeminar["VALUE"], array("sort" => "asc"), Array("CODE"=>"CITY"));
					$arRes = $db_iblock->Fetch();
					$city = $arRes["VALUE"];
					$db_iblock = CIBlockElement::GetProperty(62, $arSeminar["VALUE"], array("sort" => "asc"), Array("CODE"=>"FOR_US_P"));
					while ($ob = $db_iblock->GetNext())
					{
						if ($ob["VALUE_ENUM"])
						{
							$kto[] = "инсталляторы";
					...
					$db_iblock = CIBlockElement::GetProperty(62, $arSeminar["VALUE"], array("sort" => "asc"), Array("CODE"=>"FOR_US_U"));
					while ($ob = $db_iblock->GetNext())
					{
						if ($ob["VALUE_ENUM"])
						{
					...
					$numdays = 1;
					$db_iblock = CIBlockElement::GetProperty(46, $dayNews["ID"], array("sort" => "asc"), Array("CODE"=>"SEMINARS_COMBINED"));
					while ($ob = $db_iblock->GetNext())
				    {
				        $combined[] = $ob['VALUE'];
				    }
					$db_iblock = CIBlockElement::GetProperty(46, $dayNews["ID"], array("sort" => "asc"), Array("CODE"=>"ADDITIONAL"));
					$arRes = $db_iblock->Fetch();
					$add = $arRes["VALUE"];
					if($local == "all" || count($kto) == 0)
						$kto[] = $local;
					if(in_array($local, $kto))
					{
						$arDay["events"][] = array(
							"time"=>$eTime,
							...
							"preview"=>$sTitle,
							"city"=>$city,
							"seminar"=>$dayNews["IBLOCK_SECTION_ID"],
							"seminarID"=>$dayNews["ID"],
							"kto"=>$kto,
							"numdays"=>$numdays,
							"combined"=>$combined,
							"add"=>$add,
					...
					if (stripos($_SERVER['REQUEST_URI'], '/vebinary-po-zaprosu/')!== false)
					{
						$arDay["events"][] = array(
							"seminarID"=>$dayNews["ID"],
							"seminarDate"=>$dayNews["DATE_ACTIVE_FROM"],
						);
					}
