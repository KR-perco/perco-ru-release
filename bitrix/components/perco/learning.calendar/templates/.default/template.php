...
<div class="news-calendar">
			...
			<td class="NewsCalMonthNav" align="left">
				...
				<?if($arResult["SHOW_MONTH_LIST"]):?>
					...
					function b_result()
					{
						...
						<?if($arParams["AJAX_ID"]):?>
							jsAjaxUtil.InsertDataToNode(document.getElementById("month_sel").options[idx].value, 'comp_<?echo CUtil::JSEscape($arParams['AJAX_ID'])?>', <?echo $arParams["AJAX_OPTION_SHADOW"]=="Y"? "true": "false"?>);
						<?else:?>
	...
	<tr>
		...
		<td align="left" valign="top" class='<?=$arDay["td_class"]?>' width="14%">
			<?if($arDay[td_class]!="NewsCalOtherMonth" || $arDay[td_class]!="NewsCalOtherMonth") { ?><span class="<?=$arDay["day_class"]?>"><?=$arDay["day"]?></span><? } ?>
